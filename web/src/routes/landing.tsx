...
const DISPLAY: React.CSSProperties = { fontFamily: 'var(--togo-font-body, ui-sans-serif, system-ui, sans-serif)' };
...
function InstallBar() {
  ...
  return (
    <div className="flex items-center gap-2 h-[52px] ps-4 pe-2 rounded-md border border-border bg-card font-mono text-sm w-full sm:w-auto max-w-full min-w-0">
  ...
  return (
    <div className="bg-[#0b0f13] p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2"><img src="/togo-mark.svg?v=3" alt="" className="h-5 w-auto" /><span className="font-display text-sm font-semibold text-foreground">fort · Posts</span></div>
        <span className="rounded-md px-2 py-1 text-[11px] font-semibold text-white" style={{ background: "#1F8A99" }}>New Post</span>
      </div>
        ...
        <div className="inline-flex items-center rounded-full border border-border bg-card p-1 text-sm font-medium">
          {([["cli", "CLI"], ["ai", "✻ AI · Claude Code"]] as const).map(([m, label]) => (
            <button key={m} onClick={() => select(m)}
              className={`rounded-md px-4 py-1.5 transition-colors ${mode === m ? "text-white" : "text-muted-foreground hover:text-foreground"}`}
              style={mode === m ? { background: "#1F8A99" } : undefined}>
              {label}
            </button>
          ))}
        </div>
    ...
    <Page>
      ...
      <section className="relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0 -z-10 opacity-50"
          style={{ backgroundImage: "linear-gradient(rgba(120,140,160,.12) 1px,transparent 1px),linear-gradient(90deg,rgba(120,140,160,.12) 1px,transparent 1px)", backgroundSize: "56px 56px" }} />
      ...
      <section className="mx-auto max-w-6xl px-6 pt-16 pb-8">
        ...
        />
        <div className="border border-border overflow-hidden bg-card/40">
      ...
      <section className="mx-auto max-w-6xl px-6 pt-16 pb-6">
        ...
        <div className="mt-5 border border-border p-7 bg-card/40">
            ...
            {DBS.map((d) => (
              <span key={d.label} className="font-mono text-[13px] px-3.5 py-2 rounded-lg border"
                style={d.accent ? { borderColor: "rgba(31,138,153,.4)", color: "#1F8A99", background: "rgba(31,138,153,.07)" }
                  : d.primary ? { border: 0, background: "#1F8A99", color: "#fff" }
        ...
        <div className="border border-border overflow-hidden relative"
          ...
          <div className="p-8 sm:p-10 flex flex-col md:flex-row md:items-center gap-6 justify-between">
            <div className="max-w-xl">
              <div className="w-11 h-11 grid place-items-center mb-4 border border-[color:rgba(31,138,153,.18)] bg-[color:rgba(31,138,153,.1)] text-[var(--togo-cyan,#1F8A99)]"><Blocks size={21} /></div>
            ...
            <div className="font-mono text-[13px] text-muted-foreground bg-background/50 border border-border p-4 w-full md:w-auto">
      ...
      {/* marketplace band */}
      <section className="mx-auto max-w-6xl px-6 pt-16 pb-8">
        <div className="border border-border bg-card p-8 sm:p-10 flex flex-col sm:flex-row sm:items-center gap-6">
